...
import { assignRoles, getPageList, getRoles, removeById, removeUsers, updateOrAdd } from '@/api/acl/user'
...
export default {
  ...
  methods: {
    ...
    */
    async getRoles() {
      const result = await getRoles(this.user.id)
    ...
    */
    async assignRole() {
      ...
      this.loading = true
      const result = await assignRoles(userId, roleIds)
      this.loading = false
    ...
    */
    revomveUsers() {
      this.$confirm('确定删除吗?').then(async() => {
        await removeUsers(this.selectedIds)
    ...
    */
    async getUsers(page = 1) {
      ...
      const result = await getPageList(page, limit, searchObj)
